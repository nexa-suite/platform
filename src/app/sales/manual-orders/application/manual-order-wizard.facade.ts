...
import { ClientAccount, ClientAccountAddress, DEFAULT_CLIENT_ACCOUNT_FILTERS } from '../../client-accounts/domain/client-account.models';
import { ClientAccountAddressCommand, SalesOperationsApiService } from '../../infrastructure/http/sales-operations-api.service';
import {
...
export interface ManualOrderWizardState {
  ...
  readonly clients: readonly ClientAccount[];
...
export class ManualOrderWizardFacade {
  ...
  loadReferences(): void {
    forkJoin({
      clients: this.api.clientAccounts({ ...DEFAULT_CLIENT_ACCOUNT_FILTERS, size: 100 }),
  ...
  loadAddresses(clientId: string): void {
    this.api.clientAccountAddresses(clientId).subscribe({
  ...
  createAddress(clientId: string, command: ClientAccountAddressCommand): Observable<ClientAccountAddress> {
    return this.api.createClientAccountAddress(clientId, command).pipe(
