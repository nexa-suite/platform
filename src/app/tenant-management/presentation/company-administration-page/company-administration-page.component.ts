...
import { SegmentedControlComponent, NexaSegmentOption } from '../../../shared/presentation/components/segmented-control/segmented-control.component';
...
@Component({
  ...
  imports: [
    AccessPlanSectionComponent,
    MatButtonModule,
    MatCardModule,
    ErrorStateComponent,
    LoadingStateComponent,
    OrganizationWorkspacesSectionComponent,
    PageHeaderComponent,
    SegmentedControlComponent,
    SettingsSectionComponent,
    TeamInvitationsSectionComponent,
  ],
...
export class CompanyAdministrationPageComponent {
  ...
  readonly tabOptions = computed<readonly NexaSegmentOption[]>(() =>
    this.tabs.map((item) => ({
      value: item.id,
      label: this.i18n.t(item.labelKey),
    }))
  );
  readonly technicalReadOnly = computed(
    () =>
      !this.facade.canManageWorkspace() &&
      !this.facade.canManageRoleDefinitions() &&
      !this.facade.canManageSecurity()
  );
  ...
  friendlyMessage(value: string | null): string {
    return this.i18n.error(value);
  }
  localizedNotice(value: string | null): string {
    return value ? this.i18n.notice(value) : '';
  }
}
