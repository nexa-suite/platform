...
describe('CompanyAdministrationFacade', () => {
  ...
  it('sends workspace and membership mutations with current versions', () => {
    const membership = { id: 'member', userId: 'user', email: 'user@example.com', displayName: 'User', role: 'SALES', status: 'SUSPENDED', version: 1 };
    const mock = { organization: vi.fn(() => of(organization)), workspaces: vi.fn(() => of([{ id: 'workspace', name: 'ICISA', slug: 'icisa', status: 'ACTIVE', version: 2 }])), memberships: vi.fn(() => of([membership])), updateWorkspace: vi.fn(() => of({ id: 'workspace', name: 'ICISA 2', slug: 'icisa-2', status: 'ACTIVE', version: 3 })), changeRole: vi.fn(() => of(membership)), suspend: vi.fn(() => of(membership)), reactivate: vi.fn(() => of({ ...membership, status: 'ACTIVE' })) };
    TestBed.configureTestingModule({ providers: [CompanyAdministrationFacade, { provide: CompanyAdministrationApiService, useValue: mock }] }); const facade = TestBed.inject(CompanyAdministrationFacade); facade.load(); facade.renameWorkspace('workspace', 2, 'ICISA 2'); facade.changeRole('member', 1, 'SALES'); facade.suspend('member', 1); facade.reactivate('member', 1);
    expect(mock.updateWorkspace).toHaveBeenCalledWith('workspace', 2, { name: 'ICISA 2' }); expect(mock.changeRole).toHaveBeenCalledWith('member', 1, 'SALES'); expect(mock.suspend).toHaveBeenCalledWith('member', 1); expect(mock.reactivate).toHaveBeenCalledWith('member', 1);
  });
});
