...
export class CompanyAdministrationApiService {
  ...
  changeRole(id: string, version: number, role: string): Observable<WorkspaceMembershipSummary> { return this.http.patch<WorkspaceMembershipSummary>(platformApiUrl(this.config, `${this.base}/workspace-memberships/${encodeURIComponent(id)}/role`), { role }, { headers: this.ifMatch(version) }); }
