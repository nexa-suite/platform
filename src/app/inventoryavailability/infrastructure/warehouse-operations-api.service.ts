...
@Injectable()
export class WarehouseOperationsApiService implements WarehouseOperationsApiPort {
  ...
  release(id: string, version: number, reason: string, idempotencyKey?: string): Observable<Reservation> { return this.http.post<Reservation>(this.api(`/inventory-reservations/${encodeURIComponent(id)}/releases`), { reason }, { headers: this.commandHeaders(version, idempotencyKey) }); }
  ...
  receive(payload: { warehouseId: string; zoneId: string; catalogItemId: string; skuId?: string; batchNumber: string; expirationDate: string; quantity: number; unit: string; temperatureValue?: number; notes?: string }, idempotencyKey?: string): Observable<InventoryLot> { return this.http.post<InventoryLot>(this.api('/inventory/inbound-receipts'), payload, { headers: this.commandHeaders(undefined, idempotencyKey) }); }
  adjust(payload: { lotId: string; quantity: number; direction: 'IN' | 'OUT'; reason: string }, version: number, idempotencyKey?: string): Observable<InventoryLot> { return this.http.post<InventoryLot>(this.api('/inventory/adjustments'), payload, { headers: this.commandHeaders(version, idempotencyKey) }); }
  waste(payload: { lotId: string; quantity: number; reason: string }, version: number, idempotencyKey?: string): Observable<InventoryLot> { return this.http.post<InventoryLot>(this.api('/inventory/waste-movements'), payload, { headers: this.commandHeaders(version, idempotencyKey) }); }
  statusCommand(lotId: string, command: 'blocks' | 'quarantines' | 'availability-restorations', version: number, reason: string, idempotencyKey?: string): Observable<InventoryLot> { return this.http.post<InventoryLot>(this.api(`/inventory/lots/${encodeURIComponent(lotId)}/${command}`), { reason }, { headers: this.commandHeaders(version, idempotencyKey) }); }
  ...
  reserve(salesOrderId: string, version: number, idempotencyKey?: string): Observable<Reservation> { return this.http.post<Reservation>(this.api(`/fulfillment-candidates/${encodeURIComponent(salesOrderId)}/inventory-reservations`), {}, { headers: this.commandHeaders(version, idempotencyKey) }); }
  ...
  private commandHeaders(version?: number, idempotencyKey?: string): HttpHeaders {
    let headers = new HttpHeaders({ 'Idempotency-Key': idempotencyKey ?? crypto.randomUUID() });
    return version === undefined ? headers : headers.set('If-Match', `"${version}"`);
  }
}
