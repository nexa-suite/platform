...
export class FulfillmentReadinessPageComponent {
  ...
  select(candidate: ReadinessCandidate): void {
    ...
    this.facade.currentSalesOrderVersion(candidate.salesOrderId).subscribe({ next: (version) => { this.salesOrderVersion.set(version); this.loadingVersion.set(false); }, error: () => this.loadingVersion.set(false) });
  }
