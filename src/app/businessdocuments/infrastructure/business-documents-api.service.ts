...
export class BusinessDocumentsApiService implements BusinessDocumentsApiPort {
  ...
  download(id: string): Observable<Blob | null> {
    return this.http.get(this.api(`/business-documents/${encodeURIComponent(id)}/downloads`), { observe: 'response', responseType: 'blob' }).pipe(map((response) => response.body));
  }
  ...
  downloadEvidence(id: string): Observable<Blob | null> {
    return this.http.get(this.api(`/business-document-evidence/${encodeURIComponent(id)}/downloads`), { observe: 'response', responseType: 'blob' }).pipe(map((response) => response.body));
  }
}
