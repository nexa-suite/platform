...
export class BusinessDocumentsPageComponent {
  ...
  download(document: BusinessDocument): void {
    ...
    this.api.download(document.id).subscribe({ next: (blob) => this.save(blob, document.documentNumber || document.id, document.format.toLowerCase()), error: () => this.error.set('The document download failed.') });
  }
  ...
  downloadEvidence(evidence: EvidenceObject): void {
    ...
    this.api.downloadEvidence(evidence.id).subscribe({ next: (blob) => this.save(blob, evidence.originalFilename, ''), error: () => this.error.set('The evidence download failed.') });
  }
