...
function apiMock() {
  return {
    ...
    regionalSettings: vi.fn(() => of(regional)), unitPreferences: vi.fn(() => of(units)), securitySettings: vi.fn(() => of(security)), customFields: vi.fn(() => of([])), accessMatrix: vi.fn(() => of([])), planUsage: vi.fn(() => of(usage)), planComparison: vi.fn(() => of([])), invitations: vi.fn(() => of({ items: [], page: 0, pageSize: 25, hasNext: false })), roleDefinitions: vi.fn(() => of([])),
...
describe('CompanyAdministrationFacade', () => {
  ...
  it('loads the complete tenant administration read model', () => {
    const api = apiMock();
    TestBed.configureTestingModule({ providers: [CompanyAdministrationFacade, { provide: CompanyAdministrationApiPort, useValue: api }, { provide: PlatformAuthenticationBoundary, useValue: { hasPermission: () => true } }] });
  ...
  it('reloads the tenant read model after an ETag conflict', () => {
    const api = apiMock();
    api.changeRoles.mockReturnValue(throwError(() => ({ status: 412, code: 'CONCURRENCY_CONFLICT' })));
    TestBed.configureTestingModule({ providers: [CompanyAdministrationFacade, { provide: CompanyAdministrationApiPort, useValue: api }, { provide: PlatformAuthenticationBoundary, useValue: { hasPermission: () => true } }] });
    const facade = TestBed.inject(CompanyAdministrationFacade); facade.load();
  ...
  it('keeps the owner invariant error visible instead of treating it as a stale reload', () => {
    const api = apiMock();
    api.changeRoles.mockReturnValue(throwError(() => ({ status: 409, code: 'LAST_ACTIVE_OWNER_REQUIRED' })));
    TestBed.configureTestingModule({ providers: [CompanyAdministrationFacade, { provide: CompanyAdministrationApiPort, useValue: api }, { provide: PlatformAuthenticationBoundary, useValue: { hasPermission: () => true } }] });
  ...
  it('does not issue mutations when the required permission is absent', () => {
    const api = apiMock();
    TestBed.configureTestingModule({ providers: [CompanyAdministrationFacade, { provide: CompanyAdministrationApiPort, useValue: api }, { provide: PlatformAuthenticationBoundary, useValue: { hasPermission: () => false } }] });
    const facade = TestBed.inject(CompanyAdministrationFacade);
    facade.changeRoles('member', 1, ['WAREHOUSE']);
  ...
  it('keeps Company Owner governance permissions separate from technical tenant permissions', () => {
    ...
    const api = apiMock();
    TestBed.configureTestingModule({ providers: [CompanyAdministrationFacade, { provide: CompanyAdministrationApiPort, useValue: api }, { provide: PlatformAuthenticationBoundary, useValue: { hasPermission: (permission: string) => governed.has(permission) } }] });
    const facade = TestBed.inject(CompanyAdministrationFacade);
