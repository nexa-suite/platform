    ...
    <section class="page">
      ...
      @else if (lot(); as item) {
        <nexa-section-panel [title]="'warehouse.lots' | translate">
          ...
          <button type="button" (click)="command('blocks')" [disabled]="!facade.canWrite() || item.status === 'BLOCKED' || !adjustment.controls.reason.value">{{ 'warehouse.actions.block' | translate }}</button><button type="button" (click)="command('quarantines')" [disabled]="!facade.canWrite() || item.status === 'QUARANTINED' || !adjustment.controls.reason.value">{{ 'warehouse.actions.quarantine' | translate }}</button><button type="button" (click)="command('availability-restorations')" [disabled]="!facade.canWrite() || item.status === 'EXPIRED' || !adjustment.controls.reason.value">{{ 'warehouse.actions.restore' | translate }}</button>
        </nexa-section-panel>
        <nexa-section-panel [title]="'warehouse.actions.adjust' | translate">
          <form [formGroup]="adjustment" (ngSubmit)="adjust(item.version)"><select formControlName="direction"><option value="IN">{{ 'warehouse.forms.increase' | translate }}</option><option value="OUT">{{ 'warehouse.forms.decrease' | translate }}</option></select><input type="number" min="0.0001" formControlName="quantity" [placeholder]="'warehouse.fields.quantity' | translate"><input formControlName="reason" [placeholder]="'warehouse.fields.reason' | translate"><button type="submit" [disabled]="adjustment.invalid || !facade.canWrite()">{{ 'warehouse.actions.adjust' | translate }}</button></form>
        </nexa-section-panel>
        <nexa-section-panel [title]="'warehouse.actions.waste' | translate">
          <form [formGroup]="wasteForm" (ngSubmit)="waste(item.version)"><input type="number" min="0.0001" formControlName="quantity" [placeholder]="'warehouse.fields.quantity' | translate"><input formControlName="reason" [placeholder]="'warehouse.fields.reason' | translate"><button type="submit" [disabled]="wasteForm.invalid || !facade.canWrite()">{{ 'warehouse.actions.waste' | translate }}</button></form>
        </nexa-section-panel>
