    ...
    <section class="page">
      ...
      @else {
        <nexa-section-panel title="Create zone">
          <form [formGroup]="form" (ngSubmit)="createZone()"><input formControlName="code" placeholder="Code"><input formControlName="name" placeholder="Name"><select formControlName="type"><option value="AMBIENT">Ambient</option><option value="CHILLED">Chilled</option><option value="FROZEN">Frozen</option><option value="QUARANTINE">Quarantine</option></select><input type="number" formControlName="temperatureMin" placeholder="Min °C"><input type="number" formControlName="temperatureMax" placeholder="Max °C"><button type="submit" [disabled]="form.invalid">Create</button></form>
        </nexa-section-panel>
        @if (selected(); as zone) {
          <nexa-section-panel [title]="'Update ' + zone.code">
            <form [formGroup]="editForm" (ngSubmit)="updateZone(zone)"><input formControlName="name" placeholder="Name"><input type="number" formControlName="temperatureMin" placeholder="Min °C"><input type="number" formControlName="temperatureMax" placeholder="Max °C"><select formControlName="status"><option value="ACTIVE">Active</option><option value="SUSPENDED">Suspended</option></select><button type="submit" [disabled]="editForm.invalid">Save update</button><button type="button" (click)="clearSelection()">Cancel</button></form>
          </nexa-section-panel>
        }
        <nexa-section-panel title="Zones">
          <table><thead><tr><th>Code</th><th>Name</th><th>Type</th><th>Temperature</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            @for (zone of facade.zones(); track zone.id) { <tr><td>{{ zone.code }}</td><td>{{ zone.name }}</td><td>{{ zone.type }}</td><td>{{ zone.temperatureMin ?? '—' }} – {{ zone.temperatureMax ?? '—' }}</td><td>{{ zone.status }}</td><td><button type="button" (click)="select(zone)">Update</button><button type="button" (click)="facade.updateZone(warehouseId, zone, { status: 'SUSPENDED' })" [disabled]="zone.status !== 'ACTIVE'">Suspend</button></td></tr> }
