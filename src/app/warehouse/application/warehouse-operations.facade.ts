...
export class WarehouseOperationsFacade {
  ...
  reserve(salesOrderId: string, version: number): void { this.api.reserve(salesOrderId, version).subscribe({ next: () => this.load(), error: () => this.error.set('No se pudo reservar el pedido.') }); }
