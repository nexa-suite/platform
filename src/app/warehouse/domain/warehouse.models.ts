...
export interface Reservation { id: string; salesOrderId: string; orderNumber: string; status: string; createdAt: string; reservedAt?: string; expiresAt: string; version: number; clientAccountId?: string | null; allocations?: readonly ReservationAllocation[]; }
export interface ReservationAllocation { lotId: string; quantity: number; unit: string; expirationDate: string; }
export interface InventoryAvailability { catalogItemId: string; status: 'AVAILABLE' | 'UNAVAILABLE' | 'UNKNOWN'; asOf: string; }
export interface ReservationPreview { salesOrderId: string; orderNumber: string; complete: boolean; generatedAt: string; notice: string; lines: readonly { catalogItemId: string; requested: number; unit: string; allocations: readonly ReservationAllocation[]; shortage: number; complete: boolean; }[]; }
