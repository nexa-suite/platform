...
describe('PurchaseRequestOperationsFacade', () => {
  ...
  it('loads inbox, detail and review action', () => {
    ...
    TestBed.configureTestingModule({ providers: [PurchaseRequestOperationsFacade, { provide: SalesCommitmentApiPort, useValue: api }] }); const facade = TestBed.inject(PurchaseRequestOperationsFacade); facade.load({ ...DEFAULT_PURCHASE_REQUEST_FILTERS, status: 'SUBMITTED' }); expect(facade.state().status).toBe('success'); facade.loadDetail('PR-1'); facade.transition('PR-1', 0, 'reviews'); expect(facade.state().item?.status).toBe('IN_REVIEW');
  });
  it('exposes retryable inbox error', () => {
    ...
    TestBed.configureTestingModule({ providers: [PurchaseRequestOperationsFacade, { provide: SalesCommitmentApiPort, useValue: api }] }); const facade = TestBed.inject(PurchaseRequestOperationsFacade); facade.load(); expect(facade.state().status).toBe('error'); facade.retry(); expect(api.purchaseRequests).toHaveBeenCalledTimes(2);
  });
});
