...
import { SalesCommitmentAddressReference, SalesCommitmentCustomerReference, SalesCommitmentDeliveryAddressCommand, SalesCommitmentReferenceOption } from '../../domain/customer-reference.models';
import {
...
import { PaymentOption, PurchaseRequest, PurchaseRequestPriority } from '../../domain/purchase-requests/purchase-request.models';
import { RequestBuilderFacade } from '../../application/purchase-requests/request-builder.facade';
...
export class RequestBuilderPageComponent {
  ...
  clients(): readonly SalesCommitmentCustomerReference[] { return this.facade.state().clients; }
  catalogItems(): readonly SalesCommitmentCatalogItem[] { return this.facade.state().catalogItems; }

  selectClient(clientId: string): void {
    const client = this.clients().find((item) => item.id === clientId);
    if (client) {
      this.detailsForm.controls.deliveryProfileSnapshot.setValue(client.deliveryProfile);
      this.facade.clientAccountAddresses(client.id).subscribe({ next: (items) => { this.addresses.set(items); const address = items.find((item) => item.defaultAddress) ?? items.find((item) => item.active); if (address) this.selectAddress(address.id); } });
    }
  }
  ...
  selectAddress(addressId: string): void {
    ...
    if (address?.departmentCode) this.facade.reference('provinces', address.departmentCode).subscribe({ next: (items) => this.provinces.set(items) });
    if (address?.provinceCode) this.facade.reference('districts', address.provinceCode).subscribe({ next: (items) => this.districts.set(items) });
  }
  departmentChanged(): void { this.detailsForm.patchValue({ provinceCode: '', districtCode: '' }); const code = this.detailsForm.controls.departmentCode.value; if (code) this.facade.reference('provinces', code).subscribe({ next: (items) => this.provinces.set(items) }); }
  provinceChanged(): void { this.detailsForm.patchValue({ districtCode: '' }); const code = this.detailsForm.controls.provinceCode.value; if (code) this.facade.reference('districts', code).subscribe({ next: (items) => this.districts.set(items) }); }
  ...
  addressDisplay(address: Pick<SalesCommitmentAddressReference, 'addressType' | 'line' | 'reference' | 'departmentCode' | 'provinceCode' | 'districtCode'>): string { return [address.addressType, address.line, address.reference, address.districtCode, address.provinceCode, address.departmentCode].filter(Boolean).join(', '); }
